...
begin  -- comportamento
  ...
  p_muda_cor: process(pos_x,rstn)
		variable muda_dir: std_logic;
  begin
		if(rstn = '0') then
			color <= "000";
		elsif((pos_x = 0) or (pos_x = 127) or (pos_y = 0) or (pos_y = 95)) then
			if(color = "111") then
				color <= "001";
			else	
				color <= std_logic_vector(to_unsigned(to_integer(unsigned( color )) + 1, 3));
			end if;
		end if;
  end process p_muda_cor;
  
  pixel <= color; --when (col = pos_x) and (line = pos_y) else "000";
  
  -- O endereço de memória pode ser construído com essa fórmula simples,
  -- a partir da linha e coluna atual
  addr  <= pos_x + (128 * pos_y) when rstn = '1' else col + (128 * line);
